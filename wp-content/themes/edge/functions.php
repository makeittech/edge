...
add_action('wp_ajax_support', 'support_callback');
add_action('wp_ajax_nopriv_support', 'support_callback');
function support_callback() {
	$output = ['success' => false];

	if(isset($_POST['reason']) && isset($_POST['first_name']) && isset($_POST['last_name']) && isset($_POST['email']) && isset($_POST['message'])){
...
//                                    'send_copy_to_customer' => true
	...
	wp_send_json($output, 200); // выход нужен для того, чтобы в ответе не было ничего лишнего, только то что возвращает функция
} 